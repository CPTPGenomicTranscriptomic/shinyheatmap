...
server <- shinyServer(function(input, output) {	
  ...
  interactiveHeatmap <- reactive({
    ...
    if (n > 2000) {
      ...
      heatmaply(genexp_df_mat, scale = input$scale, dendrogram = input$dendrogram, distfun = function(x) dist(x, method = input$distanceMethod), hclustfun = function(x) hclust(x, method = input$agglomerationMethod),   k_row = 30, k_col = 4, srtCol = 45, key = input$key, cexRow = as.numeric(as.character(input$xfontsize)), cexCol = as.numeric(as.character(input$yfontsize)), Rowv = if (input$dendrogram == "both" | input$dendrogram == "row") TRUE else FALSE, Colv = if (input$dendrogram == "both" | input$dendrogram == "column") TRUE else FALSE, col = if (input$midColor == "none") colorpanel(256, low = input$lowColor, high = input$highColor) else colorpanel(256, low = input$lowColor, mid = input$midColor, high = input$highColor)) %>% layout(margin = list(l = 100, b = 200))
    }
  })
